...
from cricinfo_scraper.cricinfo_scraper.cricinfo_link_scraper import get_links
from cricinfo_scraper.cricinfo_scraper.pandas_scraper import get_all_scorecards
from cricinfo_scraper.cricinfo_scraper.batters_cleaning import clean_batters_dataframe
from cricinfo_scraper.cricinfo_scraper.batters_totals import batters_totals
...
def _store_raw_scorecard(ti):
    ...
    df.to_csv("./dags/cricinfo_scraper/data/batters.csv")
...
def _store_cleaned_scorecard(ti):
    import pandas as pd
    df: pd.DataFrame = pd.read_csv("./dags/cricinfo_scraper/data/batters.csv", index_col="Unnamed: 0")
    df = clean_batters_dataframe(df)
    df.to_csv("./dags/cricinfo_scraper/data/batters_cleaned.csv")
...
def _store_batters_totals():
    import pandas as pd
    df = pd.read_csv("./dags/cricinfo_scraper/data/batters_cleaned.csv")
    df = batters_totals(df)
    df.to_csv("./dags/cricinfo_scraper/data/batters_totals.csv")
